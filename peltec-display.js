...
export default class PelTecDisplay {
    ...
    createContent(hass, config)
    {
        ...
        return html`
            ...
            ${this.conditionalHtml(electric_heater_value == 0, this.createImage("heater.png", 70, 435, 56, null))} <!-- cc.params['B_gri'].v == 0 -->
            ${this.conditionalHtml(electric_heater_value == 1, this.createImage("grijac.png", 70, 435, 56, null))} <!-- cc.params['B_gri'].v &amp;&amp; (cc.params['B_gri'].v == 1) -->
            ${this.createText(boiler_temperature_value + " °C", 42, "color: #FFFFFF;",  100, 157)}
            ${this.createImage("senzor_b_1.png", 90, 40, 50, null)}
            ${this.createText(flue_gas_value + " °C", 26, "color: #FFFFFF;",  155, 44)}
            ${this.createImage("senzor_d.png", 425, 240, 30, null)}
            ${this.createText(mixer_temperature_value + " °C", 26, "color: #FFFFFF;", 460, 262, 80)}
            ${this.conditionalHtml(lambda_sensor_value > 0.1, this.createImage("senzor_b_1.png", 90, 90, 50, null))} <!-- (cc.params['B_Oxy1'].v &amp;&amp; (cc.params['B_Oxy1'].v > 0.1 )) -->
            ...
            ${this.createImage("vanjska.png", 600, 20, 20, "auto")}
            ${this.createText(((outdoor_temperature === false) ? "--" : outdoor_temperature) + "°C", 28, "color: #ffffff;", 530, 30)}
            ${this.conditionalHtml(tank_level_value == "Full", this.createImage("spremnikpun-n.png", 469, 330, 19, "auto"))} <!-- cc.params[ 'B_razina' ].v &amp;&amp; (cc.params['B_razina'].v == 2) -->
            ${this.conditionalHtml(tank_level_value == "Reserve", this.createImage("rezerva-n.png", 469, 330, 19, "auto"))} <!-- cc.params[ 'B_razina' ].v &amp;&amp; (cc.params['B_razina'].v == 1) -->
            ${this.conditionalHtml(tank_level_value == "Empty", this.createImage("nemapeleta-n.png", 469, 330, 19, "auto"))} <!-- cc.params[ 'B_razina' ].v &amp;&amp; (cc.params['B_razina'].v == 0) -->
            ${this.conditionalHtml(configuration_value === "4. BUF",
                ...
                ${this.createImage("akunormalno.png", this.computeX(60, 370), this.computeY(232, 570), this.computeX(165, 370), "auto")}
                </div>`)} <!-- cc.params['B_KONF'].v == 3 -->
            ${this.conditionalHtml(peltec_state !== "OFF", this.createImage("start.gif", 901, 440, 118, null))} <!-- cc.permissions === '2' &amp;&amp; cc.params['B_STATE'].v &amp;&amp; (cc.params['B_STATE'].v!=='OFF') -->
            ${this.conditionalHtml(active_command == 1 && peltec_state == "S7-3", this.createImage("pauza.png", 942, 390, 40, null))} <!-- cc.params['B_CMD'].v == 1 &amp;&amp; (cc.params['B_STATE'].v == 'S7-3') -->
