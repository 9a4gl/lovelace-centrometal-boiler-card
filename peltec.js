...
export class PelTecDisplay extends Display {
    ...
    createContent(hass)
    {
        ...
        return html`
            ...
            ${this.conditionalHtml(this.values["value"] != 0, this.createImage("unit/ventilator-unit.gif", 35, 210, 100, null))} <!-- cc.params['B_fan'].v != 0 -->
            ${this.createText(this.values["fan"], 24, "color: #000000;", 140, 255)}
            ...
            // ((cc.params["B_Tva1"].v > -45) && (cc.params["B_Tva1"].v <145))? cc.params["B_Tva1"].v : "--"
            ${this.conditionalHtml(
                'outdoor_temperature' in this.values && this.value["outdoor_temperature"] > -45 && this.value["outdoor_temperature"] < 145,
                html`${this.createImage("cmpelet/vanjska.png", 600, 20, 20, "auto")}
                     ${this.createText(this.values["outdoor_temperature"] + "°C", 28, "color: #ffffff;", 530, 30)}`)}
